...
(function ($) {
	...
	$.fn.gantt = function (options) {
		 ...
		 */
		if (options) {
			//merge the settings from html with "this" object options
		...
		var core = {
			...
			leftPanel: function (element) {
				...
				$.each(element.data, function (i, entry) {
					if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
						entries.push('<div class="row name row' + i + (entry.desc ? '' : ' fn-wide') + '" id="rowheader' + i + '" title="' + entry.name + '" offset="' + i % settings.itemsPerPage * tools.getCellSize() + '">');
			...
			// the entire chart
			createProgressBar: function (days, cls, desc, label, progress, dataObj) {
				...
				var real = $('<div class="real-bar"></div>')
					.css({
						width: (((cellWidth * days) - barMarg) + 2) * progress
					});
			...
			getMaxDate: function (element) {
				...
				switch (settings.scale) {
					...
					case "weeks":
						...
						maxDate = new Date(bd.getFullYear(), bd.getMonth() + 3, md === 0 ? 4 : md - 3);
						break;
					case "months":
			...
			getMinDate: function (element) {
				...
				switch (settings.scale) {
					...
					case "weeks":
						...
						var bd = new Date(bd.setDate(bd.getDate() - 3 * 7));
						var md = Math.floor(bd.getDate() / 7) * 7;
						minDate = new Date(bd.getFullYear(), bd.getMonth() - 3, md === 0 ? 4 : md - 3);
						break;
					case "months":
